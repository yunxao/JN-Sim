 ...
 * WARRANTIES.
 *
 * RCS: @(#) $Id: FuncSig.java,v 1.2 2003/10/24 19:36:30 Hung-ying Tyan Exp $
 *
 */
    ...
    throws TclException
{
  ...
  if (match_vector.size() == 1) {
	...
	if (!JavaInvoke.isAssignable(match_classes[j], argv_classes[j])) {
		...
		// ---DRCL---: put more efforts on matching
		try {
			if (match_classes[j].isPrimitive() && argv_classes[j] == String.class) {
				Class class_ = match_classes[j];
				String value_ = argv[startIdx + j].toString();
				if (class_ == int.class) Integer.parseInt(value_);
				else if (class_ == long.class) Long.valueOf(value_);
				else if (class_ == byte.class) Byte.valueOf(value_);
				else if (class_ == boolean.class) Boolean.valueOf(value_);
				else if (class_ == short.class) Short.valueOf(value_);
				else if (class_ == float.class) Float.valueOf(value_);
				else if (class_ == double.class) Double.valueOf(value_);
				continue;
			}
		}
		catch (Exception e_) {
