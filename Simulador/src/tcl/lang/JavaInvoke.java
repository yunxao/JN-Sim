    ...
    TclException
{
	...
	//System.out.println("JavaInvoke.call(): " + method);
    ...
    TclException		// Standard Tcl exception.
{
	...
	//System.out.println("...getsetField()1");
	...
	//System.out.println("...getsetField()2");
	...
	//System.out.println("...getsetField()3");
    ...
    TclException			// If the class cannot be found.
{
	...
	if (clsName.indexOf('.') == -1) {
		...
		// DRCL:
	    TclClassLoader tclClassLoader = null;
		try {
	   		tclClassLoader = new TclClassLoader(interp, null);
		}
		catch (SecurityException e) {
			// ignored; executed in an applet
		}

		String className_ = prefix_buf + clsName + suffix_buf;
	    try {
		// DRCL: now tclClassLoader may be null
		result = tclClassLoader == null?
			Class.forName(className_): tclClassLoader.loadClass(className_);
		//System.out.println("class: " + result);
	    } catch (ClassNotFoundException e) {
		//System.out.println("class not found: " + className_);
		...
		//System.out.println("fullyqualified: " + fullyqualified);
		...
		className_ = prefix_buf + fullyqualified + suffix_buf;
		//System.out.println("new class name: " + className_);
			// DRCL: now tclClassLoader may be null
		    result = tclClassLoader == null?
					Class.forName(className_):
					tclClassLoader.loadClass(className_);
		...
		// DRCL: now tclClassLoader may be null
	    result = tclClassLoader == null?
				Class.forName(clsName): tclClassLoader.loadClass(clsName);
	}
    } catch (ClassNotFoundException e) {
    ...
    if (result == null) {
		throw new TclException(interp, "class error \"" + clsName_buf + "\"");
    }
