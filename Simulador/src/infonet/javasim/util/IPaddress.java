...
public class IPaddress {
	...
	return (ipaddr != null &&
		...
		this.masked_val() == ((IPaddress)ipaddr).masked_val() &&
    ...
    // ----- IPaddress.hashCode ---------------------------------- //
    /**
     * Returns a hash code value which can be used if an IP address is
    ...
    // ----- IPaddress.ipprefix2bytes ---------------------------- //
    /**
     * Converts an IP address prefix to a series of bytes.  The
     * conversion may be done on either the traditional IP address
     * format or the equivalent NHI address.  If NHI addressing is
     * used, the first byte indicates the total number of bytes which
     * follow it, except in the case of the special 'bogus' address,
     * in which case the first byte is always 111, under the
     * presumption that the number of bytes required by an NHI address
     * will never be anywhere near 111.
     *
    ...
    // ----- IPaddress.bytes2ipprefix ---------------------------- //
    /**
     * Converts a series of bytes to an IP address prefix.  The
     * <code>usenhi</code> member variable determines whether or not
     * traditional IP address format or the equivalent NHI address is
     * being used.  The results are returned as a string.
     *
	...
	if (ipprefix.length() != 0)
	    throw new Error("invalid StringBuffer (must be \"\")");
	int startindex = bindex;
    ...
    /**
     * Calculate size of IPaddress if it's saved in a bytes array
     * @return nº of bytes
     */
    public int byteSize(){
    ...
    /**
     * Save information in a bytes array
     * @return
     */
    public byte[] toBytes(){
    	byte[] bytes;
     ...
     * @return
     */
    public int fromByte(byte[] bytes){
    	val = ((long)(0xff & bytes[0]) << 24) + ((long)(0xff & bytes[1]) << 16) +
	    ((long)(0xff & bytes[2]) << 8) + (long)(0xff & bytes[3]);
    	prefix_len = (int)bytes[4];
    	val &= (0xffffffff << (32-prefix_len));
    ...
    public IPaddress getMaskedIPaddress(){
    	IPaddress ipa = new IPaddress(this.masked_val(),this.prefix_len);
    	return ipa;
    }
    public IPaddress getMaskedIPaddress(int cird){
    	IPaddress ipa = new IPaddress(this.masked_val(cird),cird);
    	return ipa;
    }
