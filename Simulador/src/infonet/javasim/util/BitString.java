...
// ===== class infonet.javasim.util.BitString ==================== //
/**
 * This class implements a binary string.  It is sort of a cross
 * between a boolean array and a java.util.BitSet.  It has the fast
 * array access property of a boolean array, but the nice, intuitive
 * interface of BitSet.  (Thinking about a boolean array as a sequence
 * of bits can get annoying.)  The only bad thing is that an object of
 * this type is not nearly as space-efficient as that of a BitSet.
 */
public class BitString {
    ...
    /**
     * Converts an integer into an array of bits.  The bits are
     * represented by booleans (false=0, true=1), and the conversion
     * is done in the following way.  The lower order bits of the
     * integer will become the lower numbered elements in the bit
     * array.  If the size is specified larger than the actual number
     * of bits in the internal representation of the integer, it will
     * be assumed that these non-existent higher order bits are
     * zeroes. 
     * <p>EXAMPLE: BitString(23,8) is called.  Mentally we might
     * picture the integer 23 as the bit string 10111 (with any number
     * of 0's optionally prepended).  The size parameter tells us that
     * eight of the bits are significant to us, so for convenience
     * we'll represent 23 as 00010111.  A bit array 'b' constructed
     * from this input would be b={1,1,1,0,1,0,0,0}, or
     * b[0]=b[1]=b[2]=b[4]=1 and b[3]=b[5]=b[6]=b[7]=0, where 0=false
     * and 1=true.
     * <p>The standard printed representation of the array has the
     * most significant bits on the left and the least significant
     * bits on the right. The array representation has each index
     * corresponding to the significance of the bit it represents. In
     * other words, the element 0 corresponds to the least significant
     * bit, element 1 corresponds to the second least significant bit,
     * and so on through element <code>size-1</code>, which represents
    ...
    /**
     * Returns a substring of the bit string given one index and
     * assuming the end of the string as the other index.
     *
     * @param s  The starting index for the substring.
     * @return a substring of the bit string from the given index to
